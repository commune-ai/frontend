      ...
      <main className={classNames(classes.content, "flex flex-col items-center justify-center  my-auto ")}>
        ...
        <ul className={classes.modulesList}>
          {modulesList.map((module, i) => (
            <ModuleTile key={module.name} {...module} index={i} onClick={() => setSelectedModuleName(module.name)} />
          ))}
        </ul>
        <ModuleDetailsModal isOpen={!!selectedModuleName} onClose={() => setSelectedModuleName(null)} moduleName={selectedModuleName} />
      </main>
    </Layout>
  );
}
