...
import ModuleTreeBuilder from './moduleTree/builder'
import {IPFSTreeBuilder} from './ipfs/builder'
//********************************************************
...
export default class BlockMLPage extends Component {
  ...
  getSubPageSelection = () => {
    ...
    const margin = 30
    let buttons_list = []
    for (const [i,market_key] of ['ModuleTree', 'IPFS'/*, 'Launcher', 'Pipeline'*/].entries()) {
      buttons_list.push(
        <button className='marketsidebarbutton' style={{top:initial_top+i*(button_height+margin)}} onClick={()=> this.setState({page:market_key})}>
          {market_key}
        </button>)
  ...
  getSidebar = () => {
    ...
    if (this.state.expand_sidebar) {
      return this.getExpandedSidebar()
    } else {
  ...
  getPage = () => {
          ...
          return <div className='build'>
            {this.state.page == 'Pipeline' && <PipelineBuilder commune={commune}/>}
            {this.state.page == 'ModuleTree' && <ModuleTreeBuilder commune={commune}/>}
            {this.state.page == 'IPFS' && <IPFSTreeBuilder/>}
            </div>
        }
