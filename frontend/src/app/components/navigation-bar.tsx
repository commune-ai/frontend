...
export default function NavigationBar() {
  ...
  const handlePayWithWallet = () => {
    ...
    if (tokenType === "bitcoin") {
      const transfer = async (privateKey: string, value: number, receiver: string, env: string, address: string, ecc: any) => {
        try {
  ...
  return (
    <nav aria-label="Main" className={classes.navbar}>
      <div className={classes.items}>
        ...
        <ActiveLink activeClassName={classes.active} className={classes.item} href="https://comchat.io/">
          🥂 ComChat
        </ActiveLink>

        <Dropdown menu={{ items, onClick }}>
          <Space>
            <span style={{ fontWeight: "600", marginLeft: "0.25rem" }} className="hover:text-[#25c2a0]">
              💰 Payment
            </span>
            <DownOutlined />
          </Space>
