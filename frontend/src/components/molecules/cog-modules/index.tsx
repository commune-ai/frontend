...
const ModuleItem = ({ data }: ModuleItemPropsType) => {
  ...
  return (
    <div
      ...
      className="flex flex-col border-black  bg-white border-[1px] hover:cursor-pointer relative"
    >
      <div className="group-focus:ring flex-shrink-0">
        ...
        <div className="flex">
          <div className="p-3 flex flex-col ">
            <div>
              <h4 className="flex  group-focus:bg-black text-[22px]">
                <span className="text-gray-400 ">{data?.owner} </span>
                <span className="text-gray-400 px-1">/</span>
                <span className=" text-gray-700 truncate w-60 ">
                  {data?.name}
                </span>
              </h4>
            </div>
            <div className="text-[16px] group-focus:text-white h-20 w-60 truncate">
              {data?.description}
            </div>
          </div>
        </div>
